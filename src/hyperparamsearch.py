...
def train_transformation(epoch, hyperparams):
    print('\n=> Transformation {} Epoch: {}'.format(hyperparams[3], epoch))
    t_net.train()
    ...
    optimizer = torch.optim.Adam(params, lr=hyperparams[0]) 
    ...
    alpha, beta = hyperparams[1], hyperparams[2] # TODO : CHANGEd from 7.5, 100
    gram = GramMatrix()
    ...
    for i in range(0, len(dataloader)):
        ...
        torch.save(t_net.state_dict(), '../save/transform/trans_model_{}.ckpt'.format(hyperparams[3]))
        with open("../save/transform/info.txt", "w+") as f:
            f.write("{} {}".format(epoch, i))

        with open("../save/transform/logs/transform_train_loss{}.log".format(hyperparams[3]), "a+") as lfile:
            lfile.write("{}\n".format(train_loss))
    ...
    del dataloader
    del vdataset
    print('=> Transformation {} Network : Epoch [{}/{}], Loss:{:.4f}'.format(hyperparams[3], epoch + 1, args.epochs, train_loss))
        ...
        for al in alphas:
            for bet in betas:
                for epoch in range(0, args.epochs):
                    train_transformation(epoch, [lr, al, bet, count])
                count += 1
